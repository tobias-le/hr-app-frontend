        ...
        <Table data-testid="employees-courses">
          <TableBody>
            {currentCourses.map((course) => {
              const enrollment = course.enrolledEmployees
                .filter((enrollment) => enrollment.id.employeeId === employeeId)
                .at(0);
              return (
                <TableRow
                  data-testid="employee-course-entry"
                  key={course.learningId}
                >
                  <TableCell align="left">{course.name}</TableCell>
                  <TableCell align="right">
                    {enrollment && format(enrollment.date, "dd.MM.yyyy")}
                  </TableCell>
                </TableRow>
              );
            })}
